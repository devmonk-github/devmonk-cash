...
export class TransactionAuditUiComponent implements OnInit, OnDestroy {
  ...
  aOptionMenu: View[] = [
    {
      ...
      children: [
        // ArticleGroup
        {
          ...
          children: this.aCompactAndSpecific
        },
        {
          sKey: 'Bookkeeping',
    ...
    {
      sKey: 'sales-order',
      sValue: this.translate.instant('SALES_ORDER'),
      children: [
        // ArticleGroup
        {
          sKey: 'ArticleGroup',
    ...
    {
      sKey: 'cash-registry',
      sValue: this.translate.instant('CASH_REGISTER'),
      children: [
        // ArticleGroup
        {
          sKey: 'ArticleGroup',
  ...
  onDropdownItemSelected(parent: any, child1: any, child2: any) {
    ...
    // );
    this.selectionListItem = `${parent.sKey}->${child1.sKey}->${child2.sKey}`

  }
  setViewSelectedList() {
    this.selectionList = []
    this.aOptionMenu.forEach(a => {
      a.children.forEach(b => {
        b.children.forEach(c => {
          let selectionListItem = `${a.sKey}->${b.sKey}->${c.sKey}`
          this.selectionList.push(selectionListItem)
        })
      })
    })
  }
    ...
    let sDisplayMethod =
      this.aDisplayMethod[
        this.aDisplayMethod.findIndex(
          (el: any) => el.sKey == this.sDisplayMethod
        )
      ].sValue;
    let sType =
      this.aOptionMenu[
  ...
  expandItem(item: any, iBusinessPartnerId: string = '') {
    ...
    if (this.sDisplayMethod === 'revenuePerProperty') {
